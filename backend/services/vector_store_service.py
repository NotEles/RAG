...
class VectorStoreService:
    ...
    def _index_to_chroma(self, embeddings_data: Dict[str, Any], config: VectorDBConfig) -> Dict[str, Any]:
        ...
        try:
            ...
            collection_name = f"{base_name}{suffix}"
    ...
    def get_collection_info(self, provider: str, collection_name: str) -> Dict[str, Any]:
        ...
        if provider == VectorDBProvider.MILVUS:
            ...
            try:
                ...
                if count > 0:
                    try:
                        full_data = collection.get(limit=min(count, 1000))
                        metadatas = full_data.get('metadatas') or []
                        if isinstance(metadatas, list) and metadatas:
                            raw = max(
                                metadatas,
                                key=lambda item: str(item.get("embedding_timestamp", "")) if isinstance(item, dict) else ""
                            )
                            # 过滤掉 list 类型的字段（旧版 bug 遗留的 vector 数组）
                            schema = {k: v for k, v in raw.items() if not isinstance(v, list)}
                    except Exception as e:
                        logger.warning(f"Could not read metadata for {collection_name}: {e}")
                result = {
                    "name": collection_name,
                    "num_entities": count,
                    "schema": schema
